...
def upload_page(request: Request):
    ...
    if not user_plan:
        return RedirectResponse(url="/plan?message=upload_access&feature=Upload Documents", status_code=302)
...
def check_document(document_name: str, user_session: Optional[str] = Cookie(None)):
    ...
    try:
        ...
        try:
            ...
            try:
                existing_docs = DatabaseOperations.get_user_documents(int(user_data['user_id']))
                existing_doc = None
                
                for doc in existing_docs:
                    if doc.get('document_name', '').lower() == document_name.lower():
                        existing_doc = doc
                        break
                ...
                print(f"Database fallback error: {db_e}")
                return JSONResponse({"exists": False})
...
async def upload_file(request: Request, file: UploadFile = File(...), user_session: Optional[str] = Cookie(None)):
    """Handle file upload and processing - simplified version"""
    try:
        # Check user authentication
        user_data = verify_user_session(user_session)
        if not user_data:
        ...
        try:
            ...
            # Continue without limit check for now
        ...
        if file.content_type not in ["application/pdf", "text/plain"]:
            ...
            return templates.TemplateResponse("upload.html", get_template_context(request, "upload", error=error_msg))
        ...
        try:
            # Process file and extract chunks using vector store
            ...
            # Store in user-specific document collection using Vector Store
            ...
            # Prepare success message with vector processing info
            result_msg = f"Successfully processed {file.filename}\n"
            result_msg += f"Document Name: {document_name}\n"
            ...
            result_msg += f"Stored {num_chunks} vectors in Qdrant database\n\n"
            ...
            # Fallback to basic file storage if vector processing fails
            try:
                DatabaseOperations.save_user_document(
                    user_id=int(user_data['user_id']),
                    filename=file.filename,
                    ...
                    file_size=len(content),
                    file_type=file.content_type,
                    collection_name=f"user_{user_data['user_id']}_{document_name}",
                    tags=document_tags,
                    document_date=document_date,
                    created_date=created_date,
                    chunk_count=0
                ...
                result_msg = f"File uploaded successfully (basic mode): {file.filename}\n"
                result_msg += f"Document Name: {document_name}\n"
                result_msg += f"Vector processing error: {str(e)}\n"
                result_msg += f"File saved to database without vector indexing."
            except Exception as db_e:
                raise Exception(f"Both vector and database storage failed: {str(e)} | DB: {str(db_e)}")
        ...
        return templates.TemplateResponse("upload.html", get_template_context(request, "upload", 
            result=result_msg,
            success=True,
            user=user_data
        ))
        ...
        print(f"Upload error: {error_msg}")  # Debug logging
